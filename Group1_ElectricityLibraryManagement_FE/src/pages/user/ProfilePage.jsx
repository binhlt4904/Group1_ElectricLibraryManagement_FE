...
// import { Container, Row, Col, Card, Form, Badge, Spinner, Alert, Button } from 'react-bootstrap';
// import { PersonFill, EnvelopeFill, TelephoneFill, CalendarFill, ShieldFillCheck } from 'react-bootstrap-icons';
...
// /* Helper: luôn lấy message rõ ràng từ BE/axios error */
...
//   readValue, isEditing, inputRef, name,
// }) => (
...
//     setIsEditing(false);
//     setSaveAlert(null);
//     // uncontrolled: thoát edit là bỏ giá trị đang gõ, không cần reset
//   };
...
//     if (v) { setSaveAlert({ variant: 'danger', text: v }); return; }
...
//       // Đồng bộ lại data để hiển thị read-only sau khi lưu
//       setData((prev) => (updated && typeof updated === 'object'
//         ? { ...(prev || {}), ...updated }
//         : { ...(prev || {}), ...payload }));
...
//               {saving ? 'Saving…' : 'Save'}
//             </Button>
//             <Button variant="outline-secondary" className="ms-2" onClick={handleCancel} disabled={saving}>
//               Cancel
//             </Button>
//           </>
//         )}
...
//     const badgeText = profileType === 'READER' ? 'READER'
...
//             <h1 className={`${styles.welcomeText} text-primary`}>Welcome</h1>
//           </div>
//           <h3 className={styles.userName}>{stableName}</h3>
//           <Badge bg="secondary" className={styles.membershipBadge}>{badgeText}</Badge>
...
//               <Button as={Link} to="/admin" variant="outline-primary" size="lg" className={styles.adminButton}>
...
//                 </Form.Label>
//                 <Form.Control value={data?.username ?? ''} disabled readOnly className={styles.input} />
//               </Form.Group>
//             </Col>
//           </Row>
...
//                 </Form.Label>
//                 <Form.Control value={data?.readerCode ?? ''} disabled readOnly className={styles.input} />
//               </Form.Group>
//             </Col>
//           </Row>
//         </Form>
...
//                 </Form.Label>
//                 <Form.Control value={data?.position ?? ''} disabled readOnly className={styles.input} />
//               </Form.Group>
//             </Col>
//             <Col md={4}>
//               <Form.Group className="mb-3">
...
//                 </Form.Label>
//                 <Form.Control value={data?.joinDate ?? ''} disabled readOnly className={styles.input} />
//               </Form.Group>
//             </Col>
//             <Col md={4}>
//               <Form.Group className="mb-3">
...
//                 </Form.Label>
//                 <Form.Control value={data?.hireDate ?? ''} disabled readOnly className={styles.input} />
//               </Form.Group>
//             </Col>
//           </Row>
//         </Form>
...
//           <Button variant="outline-danger" size="sm">Change Password</Button>
//           <Button variant="outline-danger" size="sm" className="ms-2">Deactive Account</Button>
//         </div>
//       </Card.Body>
//     </Card>
//   );
...
//         </div>
//       )}
//       {!loading && err && <Alert variant="danger" className="mt-2">{err}</Alert>}
...
//             {profileType === 'UNKNOWN' && (
//               <Alert variant="warning">Unknown profile type. Please contact admin.</Alert>
//             )}
//             <AccountSettings />
//           </Col>
//         </Row>
...
const ProfilePage = () => {
  ...
  // Validate nhẹ trước khi gọi BE (profile info)
  const validate = (fullName, email, phone) => {
    if (!fullName) return 'Full name is required.';
    if (!email) return 'Email is required.';
    const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      // Clear fields sau khi đổi thành công
      if (oldPwdRef.current) oldPwdRef.current.value = '';
      if (newPwdRef.current) newPwdRef.current.value = '';
      if (confirmPwdRef.current) confirmPwdRef.current.value = '';
    } catch (e) {
